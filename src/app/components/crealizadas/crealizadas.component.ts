...
export class CrealizadasComponent implements OnInit {
  ...
  getlista() {
    ...
    if (this.model['Ifechainicio']) {
     let  fecha = this.model['Ifechainicio']['year'] +'-'+ this.model['Ifechainicio']['month'] +'-'+ this.model['Ifechainicio']['day'];
      this.model['Ifechainicio'] = fecha;
    }
    if (this.model['Ifechafin']) {
     let  fecha = this.model['Ifechafin']['year'] + "-" + this.model['Ifechafin']['month'] +'-'+ this.model['Ifechafin']['day'];
...
closemodel(content) {
  ...
  detalles(ir, alert) {
  	this.sniper = true;
  	let datos = {IDEmpresa: this.datosempresa['IDEmpresa'], token: this.token, IDValora: ir};
