...
staticAlertClosed:boolean=false;
 ...
 palabra:string="";
  ...
  delete(usuario,status){
	  	...
	  	var datos={IDEmpresa:this.datosgen["empresa"]["IDEmpresa"],Status:status,IDUsuario:usuario}
  ...
  master(usuario){
  	...
  	this.sniper=true;
  	var datos={token:this.token,IDEmpresa:this.datosempresa["IDEmpresa"],usuario};
